package util;// Wav file IO class
// A.Greensted
...
public class WavFile
{
    ...
    private List<Double> sampleData = new ArrayList<Double>();
    private double[] [] sampleDataArray ;

    // Cannot instantiate util.WavFile directly, must either use newWavFile() or openWavFile()
    private WavFile()
    {
    ...
    public double[][] getSampleDataArray () {
        return new double[(int) numFrames][2];
    }

    public List<Double> getFrameData() { return sampleData; }
    ...
    {
        if (ioState != IOState.READING) throw new IOException("Cannot read from util.WavFile instance");
        ...
        for (int f=0 ; f<numFramesToRead ; f++)
        {
            ...
            for (int c=0 ; c<numChannels ; c++)
            {
                ...
                sampleData.add((double) sampleBuffer[offset]);
                offset ++;
            }
    ...
    {
        if (ioState != IOState.WRITING) throw new IOException("Cannot write to util.WavFile instance");
...
//                if (c == 0) {
//                    sampleDataArray[offset][0] = sampleBuffer[offset];
//                } else {
//                    sampleDataArray[offset][1] = sampleBuffer[offset];
//                }
                sampleData.add(sampleBuffer[offset]);
                offset ++;
            }
